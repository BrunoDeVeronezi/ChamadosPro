...
interface OpcoesCadastroAutomaticoCliente1Props {
  isOpen: boolean;
  onClose: () => void;
  onClientCreated?: (clientId: string) => void;
}

export default function OpcoesCadastroAutomaticoCliente1({
  isOpen,
  onClose,
  onClientCreated,
}: OpcoesCadastroAutomaticoCliente1Props) {
  const { toast } = useToast();
  const [selectedOption, setSelectedOption] = useState<
    'cpf-cnpj' | 'text' | 'ocr' | 'xml' | null
  >(null);
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className='w-full max-w-5xl bg-white dark:bg-background-dark dark:border dark:border-white/10 shadow-xl rounded-xl'>
        <DialogHeader>
          <div className='flex flex-wrap items-center justify-between gap-4 p-6 border-b border-gray-200 dark:border-white/10'>
            <div className='flex flex-col gap-1'>
              <DialogTitle className='text-xl font-bold text-gray-900 dark:text-white'>
                Opções de Cadastro Automático de Cliente
              </DialogTitle>
              <DialogDescription className='text-sm font-normal text-gray-500 dark:text-gray-400'>
        ...
        <div className='grid grid-cols-1 md:grid-cols-2 gap-4 p-6'>
          {/* Card 1: Por CPF/CNPJ */}
          <Card className='flex flex-col rounded-lg border border-gray-200 dark:border-white/10 bg-white dark:bg-background-dark/50 p-6 gap-4'>
            ...
            <div className='mt-auto flex items-end gap-3 justify-between'>
              <Input
                className='w-full rounded-md border-gray-300 shadow-sm focus:border-primary focus:ring-primary dark:bg-background-dark dark:border-white/20 dark:text-white dark:placeholder-gray-500'
              ...
              />
              <Button
                className='flex shrink-0 min-w-[84px] cursor-pointer items-center justify-center overflow-hidden rounded-md h-10 px-4 bg-primary text-white text-sm font-medium leading-normal hover:bg-primary/90 focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 dark:focus:ring-offset-background-dark'
          ...
          <Card className='flex flex-col rounded-lg border border-gray-200 dark:border-white/10 bg-white dark:bg-background-dark/50 p-6 gap-4'>
            ...
            <div className='mt-auto flex items-end gap-3 justify-between'>
              <Button
                className='flex w-full cursor-pointer items-center justify-center overflow-hidden rounded-md h-10 px-4 bg-primary text-white text-sm font-medium leading-normal hover:bg-primary/90 focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 dark:focus:ring-offset-background-dark'
                onClick={() => {
